...
if game.PlaceId ~= 121864768012064 then
    game:GetService("Players").LocalPlayer:Kick("Silence only works on Fish It brahhhhh!")
    return
end
    ...
    local embed = {
        ...
        embeds = {{
            description = string.format("Congratulations **%s**! You just caught a **%s** fish!", 
                ...
                tierName),
            color = 16738740,
            author = {name = "Silence Webhook | Fish Caught"},
            ...
            footer = {text = "Powered by Silence", icon_url = "https://i.imgur.com/ly3iUKn.jpeg"},
...
InfoTab:Button({
    Title = "Need Help?",
    Desc = "Click This To Copy Discord Link.\nJoin to <font color=\"#FF90E3\">Discord Silence XD</font>!",
    Callback = function()
        if setclipboard then
            setclipboard("discord.gg/lanzstore911")
            MengHubUI:Notify({
                Title = "Success",
...
local function SetupIdentity()
    ...
    IdentityElements = {
        ...
        Watermark = overhead:FindFirstChild("SilenceWatermark")
    }
    
    if not IdentityElements.Watermark then
        IdentityElements.Watermark = Instance.new("TextLabel")
        IdentityElements.Watermark.Name = "SilenceWatermark"
        IdentityElements.Watermark.Parent = overhead
        IdentityElements.Watermark.Text = "[Silence]"
...
local function HideIdentity()
    ...
    IdentityElements.Title.Text = "Silence XD"
...
local function SavePosition(position)
    if not isfolder("Silence XD") then
        makefolder("Silence XD")
    end
    if not isfolder("Silence XD/FishIt") then
        makefolder("Silence XD/FishIt")
    end
    writefile("Silence XD/FishIt/Position.json", HttpService:JSONEncode({position:GetComponents()}))
end

local function LoadPosition()
    if isfile("Silence XD/FishIt/Position.json") then
        local success, data = pcall(function()
            return HttpService:JSONDecode(readfile("Silence XD/FishIt/Position.json"))
        end)
...
SPSection:Button({
    ...
    Callback = function()
        if isfile("Silence XD/FishIt/Position.json") then
            delfile("Silence XD/FishIt/Position.json")
        end
        MengHubUI:Notify({
                    ...
                    for _, player in ipairs(Players:GetPlayers()) do
                        if player ~= LocalPlayer then
                            ...
                            if staffRanks[rank] then
                                LocalPlayer:Kick("Silence Detected Staff, Automatically Kicked!")
                                return
                            end
                        end
                    end
...
Window:OnDestroy(function()
    ...
    print("Silence cleaned up successfully!")
end)

ConfigManager:Load()

print("Silence Loaded Successfully...")
